...
def login():
    ...
    if request.method== 'POST':
        ...
        if 'password' in request.form:
            #Check password
            login_cursor.execute("SELECT oid, password FROM logins WHERE username = '{username}'".format(username= thwart(request.form['username'])))
        ...
        #Creates a new user
            ...
            #Create new account
            login_cursor.execute('''SELECT username FROM logins where username= '{username}';'''.format(username= thwart(new_username)))
            existing_username= login_cursor.fetchall()
            login_cursor.execute('''SELECT email FROM logins where email= '{email}';'''.format(email= thwart(new_email)))
            ...
            login_cursor.execute('''INSERT INTO logins (username, password, email) 
...
def home():
    ...
    cursor.execute("SELECT sector, crag FROM boulders;")
    sectors_crags= []
